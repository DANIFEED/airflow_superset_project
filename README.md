...
## 11. Что подготовлено для команды

На первом этапе:
PostgreSQL 15 поднят и работает;
база bank_analytics создана;
датасет загружен;
таблица customers готова для использования;
внешний доступ к базе проверен.

Дальше:
Superset подключается к базе bank_analytics и строит дашборд;
Airflow реализует автоматическую загрузку новых клиентов.